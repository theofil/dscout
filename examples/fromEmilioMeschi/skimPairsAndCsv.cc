...
// minor edits by Konstantinos Theofilatos
// compile with: g++ -std=c++11 skimPairsAndCsv.cc
// run with: ./a.out /eos/cms/store/cmst3/group/daql1scout/hiion/scout_326790_000000.dat 0 dest
#include <cstdio>
#include <cstdint>
#include <cmath>
#include <string.h>
  ...
  */
  ...
  while(ibytes=fread(&header,1,sizeof(uint32_t),f)){
	...
	for(unsigned int j = i+1; j < vpt.size(); j++){
	  ...
	  if(vindex[j]<36 || vindex[j]>70){
	    continue;
	  }
	  // if(veta[j]<-1.){
	  //   std::cout << "*****************************************************************" << std::endl;
	  //   std::cout << "index: " << vindex[j] << " :ieta,ietaext " << std::hex << vieta[j] << "," << vietaext[j] << std::dec << "--" 
	  // 	      << vphi[j] << "," << vphip[j] << "," << veta[j] << ","
	  // 	      << vetap[j] << "," << vpt[j] << "," << vqual[j]
	  // 	      << "," << vcharge[j] << std::endl;
	  //   std::cout << "*****************************************************************" << std::endl;
          // }                      


	  out << bx << "," << vphi[i] << "," << vphip[i] << "," << veta[i] << "," << vetap[i] << "," << vpt[i] << "," << vqual[i] 
	      << "," << vcharge[i] << std::endl;
	  out << bx << "," << vphi[j] << "," << vphip[j] << "," << veta[j] << "," << vetap[j] << "," << vpt[j] << "," << vqual[j] 
	      << "," << vcharge[j] << std::endl;
    ...
    if(block_count%10000000==0){
      ...
      fprintf(stderr,"pairs written %d\n",eventcount);
    }
  }
  fclose(f);
