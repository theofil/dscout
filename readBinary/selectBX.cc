...
int main( int argc, char* argv[] ){
  ...
  if (argc < 3) { // We expect 3 arguments: the program name, the source path and the destination path
        ...
        std::cerr << "Compile with: g++ -std=c++11 selectBX.cc -o SelectBX.exe " << std::endl;
        std::cerr << "./SelectBX.exe /eos/cms/store/cmst3/group/daql1scout/hiion/scout_326790_000000.dat ./" << std::endl;
        std::cerr << "./SelectBX.exe /eos/cms/store/cmst3/group/daql1scout/hiion/scout_326790_000000.dat ./ 15    -> will just store 15 lines" << std::endl;
